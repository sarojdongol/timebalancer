...
class ExcerciseSelector(ttk.Frame):
  def __init__(self, parent,controller,show_settings,previous_timer):
    ...
    image = Image.open( os.path.join( Path.cwd(), 'assets/short_break_excercise/excercise_1.jpg'))
    ...
    image_button = ttk.Button(self.task_frame,
                                     text="Short Break Exercise",
                                     command=show_settings,
                                     style=(INFO, OUTLINE),
                                     cursor="hand2")
    image_button.grid(row=0,
                             column=0,
                             sticky="W")
    

    previous_timer = ttk.Button(self.task_frame,
                                  text="<- Back",
